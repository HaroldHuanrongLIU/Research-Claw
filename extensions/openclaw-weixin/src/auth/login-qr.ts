 ...
 * The OC gateway's web.login.wait handler only forwards { timeoutMs, accountId },
 * stripping the sessionKey returned by web.login.start. Using a deterministic
 * default key instead of randomUUID() lets loginWithQrWait find the active login
 * even when the sessionKey is not forwarded (the dashboard web-login flow).
 */
